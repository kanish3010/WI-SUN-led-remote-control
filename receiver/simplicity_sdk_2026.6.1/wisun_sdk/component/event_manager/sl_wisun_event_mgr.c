...
typedef enum {
  ...
  EVENT_IDX_MODE_SWITCH_FALLBACK,
  EVENT_IDX_ROUTING_TABLE_UPDATE
...
/// Notify condition storage
static app_wisun_em_notify_condition_t _conditions[APP_WISUN_EVT_MGR_MAX_CONDITIONS];

/// Notify condition mempool
...
static event_handler_t _wisun_events[] = {
  ...
  {
    .id = SL_WISUN_MSG_MODE_SWITCH_FALLBACK_IND_ID,
    .callback = sl_wisun_mode_switch_fallback_hnd,
    .custom_callback = NULL
  },
  {
    .id = SL_WISUN_BR_MSG_ROUTING_TABLE_UPDATE_IND_ID,
    .callback = sl_wisun_br_routing_table_update_hnd,
    .custom_callback = NULL
  }
};
...
void app_wisun_event_mgr_init(void)
{
  ...
  EFM_ASSERT(_app_wisun_event_mgr_mtx != NULL);
  ...
  EFM_ASSERT(sl_mempool_create(&_cond_memp,
...
void sl_wisun_on_event(sl_wisun_evt_t *evt)
{
  ...
  const app_wisun_em_notify_condition_t *cond = NULL;
  ...
  if (EVENT_IDX_NOTVALID == idx) {
    ...
    for (sl_mempool_block_hnd_t *block = _cond_memp.blocks; block != NULL; block = block->next) {
      ...
      if ((cond->id == evt->header.id)
          && (cond->condition != NULL)
          && (!cond->condition(evt))
          && (tmp_chs & (1UL << cond->evt_ch))) {
...
static void app_wisun_event_mgr_mutex_lock(void)
{
  EFM_ASSERT(osMutexAcquire(_app_wisun_event_mgr_mtx, osWaitForever) == osOK);
}

/* Mutex release */
static void app_wisun_event_mgr_mutex_unlock(void)
{
  EFM_ASSERT(osMutexRelease(_app_wisun_event_mgr_mtx) == osOK);
}
...
__STATIC_INLINE app_wisun_event_id_t _decode_ind(const sl_wisun_msg_ind_id_t ind)
{
  switch (ind) {
    ...
    case SL_WISUN_MSG_MODE_SWITCH_FALLBACK_IND_ID:          return EVENT_IDX_MODE_SWITCH_FALLBACK;
    case SL_WISUN_BR_MSG_ROUTING_TABLE_UPDATE_IND_ID:       return EVENT_IDX_ROUTING_TABLE_UPDATE;
...
/************************************************************************//**
 * @brief Wi-SUN mode switch fallback event handler
 * @details
 * @param[in] evt event ptr
 *****************************************************************************/
SL_WEAK void sl_wisun_mode_switch_fallback_hnd(sl_wisun_evt_t *evt)
{
  (void) evt;
  EFM_ASSERT(false);
}
...
/************************************************************************//**
 * @brief Wi-SUN Border Router routing table update event handler
 * @details
 * @param[in] evt event ptr
 *****************************************************************************/
SL_WEAK void sl_wisun_br_routing_table_update_hnd(sl_wisun_evt_t *evt)
{
  (void) evt;
  EFM_ASSERT(false);
}
